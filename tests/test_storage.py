...
class TestUploadDownload:
    def test_upload_and_download_round_trip(self, storage: CloudStorage, tmp_path: Path) -> None:
    ...
    def test_upload_returns_full_key(self, storage: CloudStorage, tmp_path: Path) -> None:
        src = tmp_path / "a.bin"
        src.write_bytes(b"x")
    ...
    def test_upload_rejects_nonexistent_local_file(
        self, storage: CloudStorage, tmp_path: Path
    ) -> None:
    ...
    def test_upload_rejects_directory(self, storage: CloudStorage, tmp_path: Path) -> None:
        with pytest.raises(NimbusValidationError):
            storage.upload_file(
                bucket=BucketType.DATASETS,
                project=PROJECT,
    ...
    def test_upload_to_missing_bucket_raises_storage_error(
        ...
        src = tmp_path / "x"
        src.write_bytes(b"y")
        storage.client.delete_bucket(Bucket=storage.config.bucket_name(BucketType.CHECKPOINTS))
...
class TestMultipartUpload:
    def test_multipart_upload_round_trip(self, storage: CloudStorage, tmp_path: Path) -> None:
...
class TestUploadDir:
    def test_uploads_all_files_recursively(self, storage: CloudStorage, tmp_path: Path) -> None:
    ...
    def test_missing_directory_raises(self, storage: CloudStorage, tmp_path: Path) -> None:
        with pytest.raises(NimbusValidationError):
            storage.upload_dir(
                bucket=BucketType.DATASETS,
                project=PROJECT,
...
class TestListExistsDelete:
    def _upload(
        self, storage: CloudStorage, tmp_path: Path, *, key: str, data: bytes = b"x"
    ) -> None:
        path = tmp_path / "u.bin"
    ...
    def test_list_keys_yields_keys_with_project_stripped(
        ...
        keys = sorted(storage.list_keys(bucket=BucketType.DATASETS, project=PROJECT))
        assert keys == ["a.bin", "folder/b.bin"]

    def test_list_keys_pagination(self, storage: CloudStorage, tmp_path: Path) -> None:
        for index in range(25):
            self._upload(storage, tmp_path, key=f"many/item-{index:03d}.bin")

        bucket_name = storage.config.bucket_name(BucketType.DATASETS)
        ...
        keys = sorted(
            storage.list_keys(bucket=BucketType.DATASETS, project=PROJECT, key_prefix="many/")
        )
        assert len(keys) == 25
        assert keys[0] == "many/item-000.bin"
        assert bucket_name  # silence unused warning

    def test_list_keys_with_prefix(self, storage: CloudStorage, tmp_path: Path) -> None:
        self._upload(storage, tmp_path, key="folder/a.bin")
        self._upload(storage, tmp_path, key="other/b.bin")
        keys = sorted(
            storage.list_keys(bucket=BucketType.DATASETS, project=PROJECT, key_prefix="folder/")
        )
        assert keys == ["folder/a.bin"]

    def test_exists_true_and_false(self, storage: CloudStorage, tmp_path: Path) -> None:
        self._upload(storage, tmp_path, key="present.bin")
        assert storage.exists(bucket=BucketType.DATASETS, project=PROJECT, key="present.bin")
        assert not storage.exists(bucket=BucketType.DATASETS, project=PROJECT, key="absent.bin")

    def test_delete_removes_object(self, storage: CloudStorage, tmp_path: Path) -> None:
        self._upload(storage, tmp_path, key="doomed.bin")
        storage.delete(bucket=BucketType.DATASETS, project=PROJECT, key="doomed.bin")
        assert not storage.exists(bucket=BucketType.DATASETS, project=PROJECT, key="doomed.bin")

    def test_delete_missing_is_silent(self, storage: CloudStorage) -> None:
        storage.delete(bucket=BucketType.DATASETS, project=PROJECT, key="never-existed.bin")


class TestPresignedUrl:
    def test_returns_url_with_bucket_and_key(self, storage: CloudStorage, tmp_path: Path) -> None:
        ...
        assert storage.config.bucket_name(BucketType.DATASETS) in url
        assert f"{PROJECT}/folder/object.bin" in url

    def test_invalid_expires_in_raises(self, storage: CloudStorage) -> None:
        with pytest.raises(NimbusValidationError):
...
class TestErrorMapping:
    def test_head_object_other_error_raises_storage_error(self, storage: CloudStorage) -> None:
        ...
        with pytest.raises(NimbusStorageError):
            storage.exists(bucket=BucketType.DATASETS, project=PROJECT, key="anything.bin")
